            ...
            <div className="flex ml-3 md:ml-4">
              ...
              {["L", "i", "k", "e", "d"].map((letter, index) => (
                <motion.span
                  ...
                  initial={{ y: 40, opacity: 0 }}
                  animate={{ y: 0, opacity: 1 }}
                  transition={{ duration: 0.5, delay: 0.35 + index * 0.05, ease: [0.22, 1, 0.36, 1] }}
                  className="text-3xl md:text-4xl lg:text-5xl font-medium text-[#dfff1f]"
                >
                  {letter}
                </motion.span>
...
const ActionButton = ({
  ...
  <motion.button
    ...
    onClick={onClick}
    className="p-1.5 md:p-2 bg-[#dfff1f] rounded-lg text-black hover:bg-white transition-colors relative group flex items-center gap-2"
    aria-label={label}
    tabIndex={0}
  >
    {icon}
